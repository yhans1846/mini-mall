// src/app/admin/users/page.tsx — 用户管理（等级颜色 + 等级筛选 + 搜索防抖 + 导出）
"use client";
...
const LEVEL_CONFIG = [
  { level: 0, name: "普通会员", color: "default" as const },
  { level: 1, name: "心悦1级", color: "warning" as const },
  { level: 2, name: "心悦2级", color: "info" as const },
  { level: 3, name: "心悦3级", color: "success" as const },
];

const LEVEL_NAMES = LEVEL_CONFIG.map((c) => c.name);
...
export default function AdminUsersPage() {
  ...
  const handleExport = () => {
    exportToCSV(
      ...
      [{ key: "id", label: "ID" }, { key: "name", label: "名称" }, { key: "email", label: "邮箱" },
       { key: "会员等级", label: "会员等级" }, { key: "累计消费", label: "累计消费" }, { key: "注册时间", label: "注册时间" }],
        ...
        <div className="flex flex-wrap items-center gap-3">
          <div className="relative">
            ...
            <input type="text" value={search}
              ...
              className="input-search w-64 pl-9" data-search="users" />
          </div>
          {/* 等级筛选 */}
          ...
          <table className="admin-table">
            <thead>
              <tr>
                ...
                <th>名称</th>
                <th>邮箱</th>
                <th style={{ width: 100 }} className="text-center">角色</th>
                <th style={{ width: 120 }}>会员等级</th>
            ...
            <tbody>
              {users.map((u) => {
                const levelCfg = LEVEL_CONFIG.find((c) => c.level === u.membershipLevel) || LEVEL_CONFIG[0];
                return (
                  <tr key={u.id}>
                    ...
                    <td className="text-center"><StatusBadge label="用户" type="default" /></td>
                    <td><StatusBadge label={levelCfg.name} type={levelCfg.color} /></td>
